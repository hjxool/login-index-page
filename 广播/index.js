...
new Vue({
	...
	data() {
		return {
			loginToken: '',
			userName: '',
			collector_id: '20210727_140009_1364326524935940',
			alarm_id: '20210727_140009_1277033880741141',
			static_param: {
	...
	methods: {
		...
		module_select(index) {
			switch (index) {
				case 0:
					this.request('post', collectorDetail, { id: this.collector_id }, '74935343174538', this.loginToken, (res) => {
					...
					this.request('post', alarmDetail, { id: this.alarm_id }, '74935343174538', this.loginToken, (res) => {
			...
			switch (tag) {
				case 'collector':
					if (reg.test(this.collector.name)) {
						this.request('post', update, { id: this.collector_id, name: this.collector.name }, '74935343174538', this.loginToken, () => {});
					} else {
				...
				case 'alarm':
					if (reg.test(this.alarm.name)) {
						this.request('post', update, { id: this.alarm_id, name: this.alarm.name }, '74935343174538', this.loginToken, () => {});
					} else {
						this.$message.error('名称不能为空或除了- _之外的特殊字符');
					}
					break;
